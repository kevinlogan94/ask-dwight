...
export function useSuggestionRepository() {
   ...
   */
  async function saveSuggestionsToSupabase(assistantResponseId: string, suggestions: string[]): Promise<void> {
    ...
    try {
      ...
      const { error } = await supabase.from("user_prompt_suggestions").insert(suggestionRecords as any);
