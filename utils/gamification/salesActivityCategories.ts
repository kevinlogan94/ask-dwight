...
export const CATEGORY_DETAILS: Record<SalesActivityCategory, CategoryDetails> = {
  ...
  [SalesActivityCategory.OUTREACH_WRITING]: {
    pattern:
      /\b(?:write|draft|compose|craft)\s+(?:(?:cold|initial|outreach|prospecting)\s+email|(?:linkedin|inmail)\s+message|connection\s+request(?:\s+message)?|(?:follow\s*up|check\s*in|reminder)\s+(?:email|message))\b/i,
    timeSaved: 12,
  },
  ...
  [SalesActivityCategory.CAMPAIGN_PLANNING]: {
    pattern:
      /\b(?:plan|build|create|design|develop|map\s+out|structure)\s+(?:(?:email|outreach|follow\s*up|multi\s*step|drip|nurture)\s+)?(?:sequence|cadence|campaign|series|outreach\s+plan|contact\s+strategy|sales\s+playbook)\b/i,
    timeSaved: 25,
  },
